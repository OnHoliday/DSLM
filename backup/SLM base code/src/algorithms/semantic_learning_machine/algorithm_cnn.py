...
class DeepSemanticLearningMachine(EvolutionaryAlgorithm):
    ...
    def __init__(self, population_size, stopping_criterion, layers, cnn_layers, cnn_neurons_per_layer, learning_step,
    ...
    def _create_solution(self, neural_network):
        ...
        # for neuron in neural_network.get_output_neurons(): neuron.input_connections[-1].from_node.semantics = None ###TODO ultra ważne czy wykomentowanie tego jest wazne czy nie ?!?!?!?!?!
    ...
    def _initialize_cnn_layers(self, neural_network):
        ...
        number_of_layers = randint(self.cnn_layers, self.layers)
        neurons_per_layer = [randint(1, self.cnn_neurons_per_layer) for i in range(number_of_layers - 1)]

        cnn_layers = [[create_cnn_neuron(None) for neuron in range(neurons_per_layer[layer])] for layer in range(number_of_layers - 1)]
    ...
    def _mutate_cnn_network(self):
        ...
        neural_network = copy(self.champion.neural_network)
        # Create mutated hidden layers.
        mutation_cnn_layers, mutation_hidden_layers = self.mutation_operator.mutate_network(self)
        # Connect hidden neurons to remainder of network.
        self._connect_cnn_nodes(neural_network.sensors, mutation_cnn_layers[0], full=True)
        previous_neurons = mutation_cnn_layers[0]
        for cnn_layer in mutation_cnn_layers[1:]:
            self._connect_cnn_nodes(previous_neurons, cnn_layer, full=False)
            previous_neurons = cnn_layer
        i = 0
        for layer in mutation_cnn_layers:
            if i == 0:
                for neuron in layer:
                    neuron.calculate()
            else:
        ...
        # Extend cnn layers.
        for cnn_layer, mutation_cnn_layers in zip(neural_network.cnn_layers, mutation_cnn_layers):
            cnn_layer.extend(mutation_cnn_layers)

        # Extend flatten layers.
        neural_network.flatten_layer.extend(flatten_layer_mutated)

        # Extend hidden layers.
        for hidden_layer, mutation_hidden_layers in zip(neural_network.hidden_layers, mutation_hidden_layers):
            hidden_layer.extend(mutation_hidden_layers)
        # Connect final hidden neuron to output neuron.
        self._connect_learning_step(neural_network)
        # Get most recent connection.
        for neuron in neural_network.output_layer:
    ...
    def fit(self, input_matrix, target_vector, metric, verbose=False):
        ...
        self.champion.neural_network = deepcopy(self.champion.neural_network) ###TODO Potrzebne zeby policzyc finalne wartosci dla test setu !!!!
